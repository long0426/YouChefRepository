    ...
    var app = current.AddressSeleclList = 
    {
        AdrressArray: [
                        ...
                        ['台南市', '700中西區', '701東區', '702南區', '704北區', '708安平區', '709安南區', '710永康市', '711歸仁區', '712新化區', '713左鎮區', '714區井區', '715楠西區', '716南化區', '717仁德區', '718關廟區', '719龍崎區', '720官田區', '721麻豆區', '722佳里區', '723西港區', '724七股區', '725將軍區', '726學甲區', '727北門區', '730新營區', '731後壁區', '732白河區', '733東山區', '734六甲區', '735下營區', '736柳營區', '737鹽水區', '741善化區', '741新市區', '742大內區', '743山上區', '744新市區', '745安定區'],
        ...
        defaultOptionDistrictText: '請選擇鄉鎮',
        defaultOptionDistrictValue: '',

        Initialize: function (city, district, defaultCityText, defaultCityValue, defaultDistrictText, defaultDistrictValue) {

            var cityText = defaultCityText ? defaultCityText : this.defaultOptionCityText;
            var cityValue = defaultDistrictValue ? defaultDistrictValue : this.defaultOptionCityValue;
            var districtText = defaultDistrictText ? defaultDistrictText : this.defaultOptionDistrictText;
            var districtValue = defaultDistrictValue ? defaultDistrictValue : this.defaultOptionDistrictValue;

            var citySelect = document.getElementById(city);
            var districtSelect = document.getElementById(district);

            citySelect.options[0] = new Option(cityText, cityValue);
            districtSelect.options[0] = new Option(districtText, districtValue);
            for (var i = 0; i < this.AdrressArray.length; i++) {
                citySelect.options[i + 1] = new Option(this.AdrressArray[i][0], this.AdrressArray[i][0]);
            }
            citySelect.addEventListener ? citySelect.addEventListener('change', function (e) { app.AppendDistrict(e, districtSelect, districtText, districtValue) }, false) : citySelect.attachEvent('onchange', function (e) { app.AppendDistrict(e, districtSelect, districtText, districtValue) });
        },

        AppendDistrict: function (e, DistrictSelect, districtText, districtValue) {
            ...
            DistrictSelect.options.length = this.AdrressArray[target.selectedIndex - 1].length - 1;
            for (var i = 1; i < this.AdrressArray[target.selectedIndex - 1].length; i++) {
                DistrictSelect.options[i - 1] = new Option(this.AdrressArray[target.selectedIndex - 1][i], this.AdrressArray[target.selectedIndex - 1][i]);
            }
        },

        ReturnSelectAddress: function (city, district, address) {
            var city = document.getElementById(city);
            var district = document.getElementById(district);
            var address = document.getElementById(address)
            return city.value + district.value + address.value;
        	}
   		 };
		})(window);