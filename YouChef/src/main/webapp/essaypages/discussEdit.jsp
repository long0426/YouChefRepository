	...
	<c:forEach var="discuss" items="${discussList}">
		<c:choose>
			<c:when test="${discuss.discuss_id == param.discuss_id }">
				<form action="<c:url value="${request.contextPath}/discuss/updatediscuss.controller"/>" method="POST">
					<div>
					...
					<div>
							...
							<textarea cols="40" rows="20" name="content">${discuss.content}</textarea>
					</div>			
					<p>
					<input type="button" onclick="history.go(-1)" name="getessay" value="取消編輯" >
					<input type="submit" name="inessay" value="編輯完成">
				</form>
			</c:when>
		</c:choose>
	</c:forEach>
