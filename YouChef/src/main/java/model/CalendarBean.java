...
public class CalendarBean implements java.io.Serializable {
	...
	public String toString() {
		Integer[] arr = { date1, date2, date3, date4, date5, date6, date7, date8, date9, date10, date11, date12, date13,
				date14, date15, date16, date17, date18, date19, date20, date21, date22, date23, date24, date25, date26,
				date27, date28, date29, date30, date31 };
		...
		if (null != chefBean) {
			for (int i = 0; i < arr.length; i++) {
				if (4 != arr[i]) {
					...
					sb.append(theMonth.substring(0, 4)).append("-").append(theMonth.substring(4)).append("-")
							.append(String.format("%02d", i + 1)).append("_").append(" ").append(start_time).append("_")
							.append(" ").append(end_time).append("_").append(title).append("_").append(eventColor)
							.append(",");
				}
			}
		}else if (null != mchefBean) {
			for (int i = 0; i < arr.length; i++) {
				if (maxNum != arr[i]) {
					switch (arr[i]) {
					case -1:
	...
	private boolean checkDate(String time) {
		...
		date.set(Integer.parseInt(time.substring(0, 4)), Integer.parseInt(time.substring(4, 6)) - 1,
