...
@Controller
@RequestMapping(path = { "/pages/checkmchefalong.controller" })
public class CheckMchefAlongController {
	@Autowired
	...
	public void process(Model model, HttpServletRequest request, HttpServletResponse response) {
		...
		CalendarBean cb = calendarService.selectMchef(Integer.parseInt(mc_id), date);
		response.setContentType("text/html;chartset=UTF-8");
		try {
			if(null != cb && Integer.parseInt(quota) != getDate(cb, day)){
